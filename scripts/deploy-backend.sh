...
if test "$slim_mode" = "--dry-run"; then echo "DRY RUN — 不会连接服务器"; echo "目标：$slim_user@$slim_host:$slim_dir"; echo "将同步 backend 与 deploy 配置，再由 Docker Compose 构建、迁移并重启服务。"; exit 0; fi
...
ssh "$DEPLOY_USER@$DEPLOY_HOST" "cd '$DEPLOY_DIR' && docker compose -f deploy/docker-compose.backend.yml up -d --build && docker compose -f deploy/docker-compose.backend.yml exec -T slimming-api alembic upgrade head"
