...
const { buildMealSections, buildPregnancyTodayModel } = require("../../utils/meal-plan");

Page({
  data:{dateLabel:"",pregnancy:null,gestationLabel:"",currentWeightKg:null,sections:[],feelingCodes:[],wellbeingOptions:[{code:"normal",label:"状态还好"},{code:"nausea",label:"有些恶心"},{code:"reflux",label:"反酸"},{code:"constipation",label:"便秘"},{code:"low_appetite",label:"胃口较低"}],aiTip:"只基于已确认记录提供有限回顾。"},
  onLoad(){const now=new Date();this.setData({dateLabel:`${now.getMonth()+1}月${now.getDate()}日 · 今天`});},onShow(){this.load();},
  async load(){const today=localDateKey();try{const [pregnancy,plan,meals,schedules,wellbeing]=await Promise.all([getCurrentPregnancy(),getMealPlan(today),listMeals(today),listMealSchedules(),getWellbeing(today)]);const model=buildPregnancyTodayModel(pregnancy,plan),codes=wellbeing.feeling_codes||[];this.setData({pregnancy,gestationLabel:model.gestationLabel,currentWeightKg:model.currentWeightKg,sections:buildMealSections(plan,meals,schedules),feelingCodes:codes,wellbeingOptions:this.data.wellbeingOptions.map(item=>({...item,selected:codes.indexOf(item.code)>=0})),aiTip:meals.items&&meals.items.length?"今天的记录已纳入周期回顾，所有营养数值来自已确认食物。":"先记录一餐，周期回顾只会总结你已确认的数据。"});}catch(error){if(error&&error.statusCode===404)wx.reLaunch({url:"/pages/onboarding/welcome/index"});else console.warn("load pregnancy today",error);}},
